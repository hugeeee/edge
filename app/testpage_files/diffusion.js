DiffusionClient=new function(){this.version="4.6.15";this.buildNumber="01";this.isInvalidFunction=false;
...
};this.createTopicNotifyTopicHandler=function(a,b){return new TopicNotifyTopicHandler(a,this,b)};this.removeTopicListener=function(b){var c;
for(var a=0;a<this.listeners.length;a++){c=this.listeners[a];if(c.getHandle()==b){this.listeners.splice(a,1);
return}}};this.removeTimedTopicListener=function(b){var c;for(var a=0;a<this.listeners.length;a++){c=this.listeners[a];
if(c.getHandle()==b){c.stop();this.listeners.splice(a,1);return}}};this.removeAllTopicListeners=function(){this.listeners=[]
};this.checkEscape=function(a){if(!a){a=event}if(a.keyCode==27){return false}};this.extend=function(c,a){for(var b in a){c[b]=a[b]
}return c};this.bind=function(a,b){return function(){var c=Array.prototype.slice.call(arguments,0);return a.apply(b,c)
}};this.getClientID=function(){return this.diffusionTransport.clientID};this.getClientProtocolVersion=function(){return 4
...
};DiffusionWSTransport.prototype.connect=function(a){var f=DiffusionClient.connectionDetails;if(f.disableWS){DiffusionClient.diffusionTransport.cascade();
return}this.hasConnected=false;DiffusionClient.trace("WebSocket connect");try{var b="";if(f.topic!==null){b="t="+encodeURIComponent(f.topic)+"&"
}var d=f._wsURL()+f.context+"/diffusion?"+b+"v="+DiffusionClient.getClientProtocolVersion()+"&ty=WB";
if(a){d+="&c="+DiffusionClient.getClientID()}if(DiffusionClient.credentials!=null){d+="&username="+encodeURIComponent(DiffusionClient.credentials.username)+"&password="+encodeURIComponent(DiffusionClient.credentials.password)
}DiffusionClient.trace("WebSocket URL: "+d);var c=DiffusionClient.getWebSocket(d);if(c==null){DiffusionClient.diffusionTransport.cascade();
return}this.webSocket=c;this.webSocket.onopen=DiffusionClient.bind(this.onWSConnect,this);this.webSocket.onclose=DiffusionClient.bind(this.onWSClose,this);
this.webSocket.onmessage=DiffusionClient.bind(this.onWSHandshake,this);this.timeoutVar=setTimeout(DiffusionClient.bind(this.onTimeout,this),f.wsTimeout)
}catch(g){DiffusionClient.trace("WebSocket connect exception "+g);clearTimeout(this.timeoutVar);DiffusionClient.diffusionTransport.cascade();
...
}if(DiffusionClient.diffusionTransport.isClosing!=true){DiffusionClient.diffusionTransport.isClosing=true;
...
this.clearPlugin();var e=DiffusionClient.connectionDetails.context+c.libPath+"/DiffusionClient.swf?v=4.6.15_01&host="+c._flashHost()+"&port="+c._flashPort()+"&topic="+encodeURIComponent(c.topic);
...
e+="&transport="+c.flashTransport+"&durl="+c._flashURL()+"&tio="+c.flashTimeout;if(c.flashSecure===true){e+="&secure=true"
}if(DiffusionClient.credentials!=null){e+="&username="+encodeURIComponent(DiffusionClient.credentials.username)+"&password="+encodeURIComponent(DiffusionClient.credentials.password)
...
if(window.chrome){this.silverlightObjectDelay=1200}else{this.silverlightObjectDelay=200}};DiffusionSilverlightTransport.prototype.close=function(){if(this.silverlightConnection!=null){this.silverlightConnection.close()
...
if(this.clearPlugin()){var a=this.silverlightObjectDelay}else{var a=0}var d=this;setTimeout(function(){d.createSilverlightObject(b)
},a)};DiffusionSilverlightTransport.prototype.createSilverlightObject=function(b){var c=DiffusionClient.connectionDetails;
var h=DiffusionClient.connectionDetails.context+c.libPath+"/DiffusionClient.xap?v=4.6.15_01";var d='<object data="data:application/x-silverlight-2," id="DiffusionClientSilverlight" type="application/x-silverlight-2" width="1" height="1">';
d+='<param name="source" value="'+h+'" />';d+='<param name="onError" value="DiffusionClient.diffusionTransport.transport.onSilverlightError" />';
var g;if(c.topic!=null){g=c.topic.split(",").join("|")}else{g=""}var f="host="+c._silverlightHost()+",port="+c._silverlightPort()+",topic="+g+",onDataEvent=DiffusionClient.diffusionTransport.handleMessages,";
f+="callback=DiffusionClient.diffusionTransport.transport.onSilverlightConnect,transport="+c.silverlightTransport+",durl="+c._silverlightURL();
...
a.removeChild(b);return true}}catch(d){Diffusion.trace("Unable to clear previous Silverlight object")
}return false};DiffusionSilverlightTransport.prototype.onSilverlightConnect=function(b){clearTimeout(this.timeoutVar);
...
}e+="MethodName: "+a.methodName+"\n"}}DiffusionClient.trace(e)};DiffusionXHRTransport=function(){this.serverUrl=DiffusionClient.connectionDetails._XHRURL()+DiffusionClient.connectionDetails.context+"/diffusion/";
...
};DiffusionXHRTransport.prototype.sendAckResponse=function(a){this.processRequest(this.createDiffusionRequest({m:"32",c:DiffusionClient.getClientID(),a:a,s:this.seq++}))
...
};DiffusionXHRTransport.prototype.close=function(){var a=this.createXHRTransport();a.open("POST",this.serverUrl,false);
a.setRequestHeader("m","29");a.setRequestHeader("c",DiffusionClient.getClientID());try{a.send("")}catch(b){}if(this.pollRequest){this.aborted=true;
this.pollRequest.abort()}};DiffusionXHRTransport.prototype.poll=function(){if(DiffusionClient.diffusionTransport.isClosing){return
...
};DiffusionIframeTransport.prototype.sendAckResponse=function(a){this.post("?m=32&c="+DiffusionClient.getClientID()+"&a="+a)
...
}if(this.pollFrame){this.container.removeChild(this.pollFrame);this.pollFrame=null}this.container.removeChild(this.connectFrame);
...
this.cascadeTimeout=4000;this._defaultFlashHost=window.location.hostname;this.flashHost=this._defaultFlashHost;
this._defaultFlashPort=((window.location.port==="")?80:parseInt(window.location.port));this.flashPort=this._defaultFlashPort;
this.flashSecure=location.protocol==="https:"?true:false;this._defaultFlashURL=location.protocol+"//"+location.host;
this.flashURL=this._defaultFlashURL;this.flashTransport="S";this.flashTimeout=3000;this.disableFlash=false;
this._defaultSilverlightHost=window.location.hostname;this.silverlightHost=this._defaultSilverlightHost;
this._defaultSilverlightPort=4503;this.silverlightPort=this._defaultSilverlightPort;this._defaultSilverlightURL=location.protocol+"//"+location.host;
this.silverlightURL=this._defaultSilverlightURL;this.silverlightTransport="S";this.disableSilverlight=false;
this._defaultXHRURL=location.protocol+"//"+location.host;this.XHRURL=this._defaultXHRURL;this.XHRretryCount=3;
this.timeoutMS=4000;this.transportTimeout=90;this.disableXHR=false;this._defaultWsURL="ws";if(location.protocol=="https:"){this._defaultWsURL+="s"
}this._defaultWsURL+="://"+location.host;this.wsURL=this._defaultWsURL;this.wsTimeout=2000;this.disableWS=false;
...
this.onServerRejectedCredentialsFunction=null;this.onTopicStatusFunction=null;this.serverHost=window.location.hostname;
this.serverPort=((window.location.port==="")?80:parseInt(window.location.port))};DiffusionClientConnectionDetails.prototype._serverHost=function(){if(typeof this.serverHost==="string"){return serverHost
}else{if(typeof this.serverHost==="object"){if(this.serverHost instanceof String){return serverHost.valueOf()
}else{return serverHost.toString()}}else{return serverHost.toString()}}};DiffusionClientConnectionDetails.prototype._serverPort=function(){if(typeof this.serverPort==="number"){return this.serverPort
}else{if(typeof this.serverPort==="string"){return parseInt(this.serverPort)}else{if(typeof this.serverPort==="object"){if(this.serverPort instanceof Number){return this.serverPort.getValue()
}else{if(this.serverPort instanceof String){return parseInt(this.serverPort)}else{return this.serverPort
}}}else{return this.serverPort}}}};DiffusionClientConnectionDetails.prototype._wsURL=function(){if(this.wsURL==this._defaultWsURL){if(location.protocol==="http:"){var a="ws"
}else{var a="wss"}a=a+"://"+this.serverHost;if(this._serverPort()!=80){a=a+":"+this._serverPort()}return a
}else{return this.wsURL}};DiffusionClientConnectionDetails.prototype._flashHost=function(){if(this.flashHost==this._defaultFlashHost){return this.serverHost
}else{return this.flashHost}};DiffusionClientConnectionDetails.prototype._flashPort=function(){if(this.flashPort==this._defaultFlashPort){return this._serverPort()
}else{return this.flashPort}};DiffusionClientConnectionDetails.prototype._flashURL=function(){if(this.flashURL==this._defaultFlashURL){if(this._serverPort()==80){return location.protocol+"//"+this.serverHost
}else{return location.protocol+"//"+this.serverHost+":"+this._serverPort()}}else{return this.flashURL
}};DiffusionClientConnectionDetails.prototype._silverlightHost=function(){if(this.silverlightHost==this._defaultSilverlightHost){return this.serverHost
}else{return this.silverlightHost}};DiffusionClientConnectionDetails.prototype._silverlightPort=function(){if(this.silverlightPort==this._defaultSilverlightPort){if(this._serverPort()>=4502&&this._serverPort()<=4534){return this._serverPort()
}}return this.silverlightPort};DiffusionClientConnectionDetails.prototype._silverlightURL=function(){if(this.silverlightURL==this._defaultSilverlightURL){if(this._serverPort()==80){return location.protocol+"//"+this.serverHost
}else{return location.protocol+"//"+this.serverHost+":"+this._serverPort()}}else{return this.silverlightURL
}};DiffusionClientConnectionDetails.prototype._XHRURL=function(){if(this.XHRURL==this._defaultXHRURL){if(this._serverPort()==80){return location.protocol+"//"+this.serverHost
}else{return location.protocol+"//"+this.serverHost+":"+this._serverPort()}}else{return this.XHRURL}};
DiffusionRecord=function(){this.fields=[];DiffusionRecord.prototype.addFields.apply(this,arguments)};
DiffusionRecord.prototype.fieldDelimiter="\u0002";DiffusionRecord.prototype.recordDelimiter="\u0001";
DiffusionRecord.prototype.setField=function(a,b){if(this.fields[a]){this.fields[a]=b}};DiffusionRecord.prototype.addField=function(a,b){if(a>=this.fields.length){this.fields.push(b)
}else{this.fields.splice(a,0,b)}};DiffusionRecord.prototype.addFields=function(d){if(d){var a=typeof d==="string"?arguments:d;
var c=a.length,b=0;for(;b<c;++b){this.fields.push(a[b])}}};DiffusionRecord.prototype.removeField=function(a){this.fields.splice(a,1)
...
};TopicListener.prototype.callFunction=function(a){try{a.rewind();if(this.fp.apply(this.context,[a])==true){return true
...
}}};TopicDataType=function(b,a){this.code=b;this.description=a};TopicDataType.NONE=new TopicDataType("N","None");
TopicDataType.SINGLE_VALUE=new TopicDataType("S","Single");TopicDataType.RECORD=new TopicDataType("R","Record");
TopicDataType.PROTOCOL_BUFFER=new TopicDataType("G","Google protocol buffer");TopicDataType.CUSTOM=new TopicDataType("U","Custom");
TopicDataType.SLAVE=new TopicDataType("SD","Slave");TopicDataType.SERVICE=new TopicDataType("SV","Service");
TopicDataType.PAGED_STRING=new TopicDataType("PS","Paged string");TopicDataType.PAGED_RECORD=new TopicDataType("PR","Paged record");
TopicDataType.TOPIC_NOTIFY=new TopicDataType("TN","Topic notify");TopicDataType.ROUTING=new TopicDataType("RO","Routing");
TopicDataType.CHILD_LIST=new TopicDataType("C","Child");TopicDataType.REMOTE_CONTROL=new TopicDataType("RC","Remote control");
TopicDataType.REMOTE_SERVICE=new TopicDataType("RS","Remote service");TopicDataType.REMOTE_CONTROL_RELAY=new TopicDataType("RCR","Remote control");
TopicDataType.REMOTE_SERVICE_RELAY=new TopicDataType("RSR","Remote service");TopicDataType.MAP={N:TopicDataType.NONE,S:TopicDataType.SINGLE_VALUE,R:TopicDataType.RECORD,G:TopicDataType.PROTOCOL_BUFFER,U:TopicDataType.CUSTOM,SD:TopicDataType.SLAVE,SV:TopicDataType.SERVICE,PS:TopicDataType.PAGED_STRING,PR:TopicDataType.PAGED_RECORD,TN:TopicDataType.TOPIC_NOTIFY,RO:TopicDataType.ROUTING,C:TopicDataType.CHILD_LIST,RC:TopicDataType.REMOTE_CONTROL,RS:TopicDataType.REMOTE_SERVICE,RCR:TopicDataType.REMOTE_CONTROL_RELAY,RCR:TopicDataType.REMOTE_SERVICE_RELAY};
NotificationLevel=function(a){this.code=a};NotificationLevel.MINIMAL=new NotificationLevel("0");NotificationLevel.PROPERTIES=new NotificationLevel("1");
NotificationLevel.METADATA=new NotificationLevel("2");NotificationLevel.FULL=new NotificationLevel("3");
NotificationLevel.NONE=new NotificationLevel("4");SelectionMode=function(a){this.code=a};SelectionMode.ADD=new SelectionMode("A");
SelectionMode.REMOVE=new SelectionMode("D");SelectionMode.REPLACE=new SelectionMode("R");SelectionMode.CLEAR=new SelectionMode("X");
TopicMetadata=function(b,a){this.name=b;this.xml=a};TopicMetadata.CACHE=new Object();TopicMetadata.getTopicMetadata=function(b,a){if(a===undefined){return TopicMetadata.CACHE[b]
}else{var c=new TopicMetadata(b,a);TopicMetadata.CACHE[b]=c;return c}};TopicDefinition=function(a,c,b){this.dataType=a;
this.properties=c;this.metadata=b};TopicNotifyTopicListener=function(){};TopicNotifyTopicListener.prototype.topicAdded=function(a,b){};
TopicNotifyTopicListener.prototype.topicRemoved=function(a){};TopicNotifyTopicListener.prototype.topicUpdated=function(a,b){};
TopicNotifyTopicHandler=function(b,a,c){this.client=a;this.notificationListener=c;this.topicName=b.getTopic();
this.topicListener=a.addTopicListener(this.topicName,this.handleTopicMessage,this)};TopicNotifyTopicHandler.prototype.setNotificationDetails=function(c,a,d){var b=new TopicMessage(this.topicName,null);
b.setUserHeaders([c.code,"true",a,d]);this.client.command("L",null,b)};TopicNotifyTopicHandler.prototype.select=function(c,a){var b=new TopicMessage(this.topicName,null);
b.setUserHeaders([c.code,a]);this.client.command("S",null,b)};TopicNotifyTopicHandler.prototype.handleTopicMessage=function(a){var b=a.getUserHeaders();
if(b.length===1){if(a.notificationType==="A"){this.notificationListener.topicAdded(b[0],this.parseAddedNotification(a))
}else{if(a.notificationType==="D"){this.notificationListener.topicRemoved(b[0])}else{if(a.notificationType==="U"){this.notificationListener.topicUpdated(b[0],this.parseUpdatedNotification(a))
}}}}};TopicNotifyTopicHandler.prototype.parseAddedNotification=function(g){var c=g.getRecords();var b=null;
var f=null;var e=null;if(c.length>0){b=TopicDataType.MAP[c[0].getField(0)];if(c.length>1){var a=c[1].getFields();
if(a.length>1){f=new Object();for(var d=0;d<a.length;d=d+2){f[a[d]]=a[d+1]}}if(c.length>2){if(c[2].getFields().length===3){if(c[2].getField(0)==="X"){e=TopicMetadata.getTopicMetadata(c[2].getField(1),c[2].getField(2))
}}else{if(c[2].getFields().length===2){if(c[2].getField(0)==="R"){e=TopicMetadata.getTopicMetadata(c[2].getField(1))
}}}}}}return new TopicDefinition(b,f,e)};TopicNotifyTopicHandler.prototype.parseUpdatedNotification=function(e){var b=e.getRecords();
var d=new Object();var a=b[0].getFields();if(a.length>1){d=new Object();for(var c=0;c<a.length;c=c+2){d[a[c]]=a[c+1]
}}return d};